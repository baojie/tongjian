...
```bash
# 提取页面中所有引文，逐条验证
python3 wiki/scripts/butler/check_citations.py wiki/public/pages/商鞅.md
```
...
**验证方法**：
```bash
# 检查页面中的结论性陈述是否有 PN 标注支撑
grep -c "（\d{3}-\d{3}）" wiki/public/pages/商鞅.md
```
...
```bash
python3 wiki/scripts/butler/check_citations.py --fix-critical wiki/public/pages/商鞅.md
```

> ⚠️ **编辑接口规范（W0 不变量）**：所有修改 `wiki/public/pages/*.md` 的操作，

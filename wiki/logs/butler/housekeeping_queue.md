...
### W5 反思 (R10025, >50轮强制)
> 全库状态扫描与模式分析（上次W5: R9628，间隔397轮，远超50轮触发强制W5）

- **轮次**: R9628—R10025, 397轮, accept 397/397 (100%)
  - W10e corpus_search enrich（刘攽主导，~93轮，12页/轮，~22%命中率）
  - 0bq→bq person enrich（范祖禹主导，~50轮，5页/轮）
  - 周期任务全部正常执行（H20/H21/H22/H17/H18）

- **库规模**: 17422页（与R9628一致，冻结）
  - quality: standard 4690 / featured 604 / basic 1339 / stub 86 / none 10703
  - featured pages: 40（不变）
  - 正文平均: ~774B，中位数: ~670B
  - 缺 dynasty: ~21.5%（采样估算，较R9628的24.3%略降）

- **W10e 进展（R10008-R10025，刘攽）**:
  - 累计93轮，~1116页 enriched with corpus_search blockquotes
  - 扫描命中率稳定在16-52%（平均~22%）
  - 池深度稳定：每80页扫描命中16-20页，选top12 enrich
  - 候选类型多样：官职/概念/地名/事件/人物全覆盖

- **实例竞争（最近30轮）**:
  - 刘攽(18/30, 60%) — W10e enrich
  - 范祖禹(12/30, 40%) — 0bq→bq person enrich
  - 两实例和谐共存，交替占锁成功

- **周期任务收敛状态**:
  - H20 wikilink-pass → 接近饱和（每轮2-30链接）
  - H21 person-table-scan → 完全收敛（0错误）
  - H22 fix-emdash → 完全收敛（0/50修改）
  - H18 stub-triage → 完全收敛（0 person<500B）
  - H23/H24 → 已完成，无积压

- **暂存状态**: 315文件（自R9991发布后积压，约+3007/-33行）
- **最近发布**: R9991（2d43ed2b8），距下次R10097（%97=0）尚有~72轮

- **观察A**: W10e corpus_search enrich 稳定运行93轮，命中率稳定，池未见枯竭迹象
- **观察B**: 范祖禹 0bq→bq 补充人物页 blockquote，与 W10e 形成双轨 enrich
- **观察C**: 所有周期任务已达收敛终端（H21/H22/H18/H20），无积压待办
- **观察D**: 库规模冻结于17422页（无create-page），纯质量提升模式
- **观察E**: 暂存315文件积压正常，等待R10097发布清账

- **建议**:
  1. 继续 W10e corpus_search enrich 为主策略（池深度充足）
  2. R10097（%97=0）执行 /wiki publish 清积压
  3. dynasty 字段批量填充仍待规划（~3750页缺）
  4. 候选类型偏薄（官职/概念为主），可考虑扩大扫描范围

### W5 反思 (R9657, %29=0)
> 全库状态扫描与模式分析（上次W5: R9628，间隔29轮）

- **轮次**: R9628—R9656, 28轮, accept 28/28 (100%)
  - ic⊈bp partial cleanup（主导，5页/轮稳定消耗）
  - H20 wikilink-pass×1（R9646: 2页/30链接，接近饱和）
  - H21 person-table-scan×1（R9653: 0错误，完全收敛）
  - 周期任务全部正常执行

- **库规模**: 17422页（与R9628一致）
  - quality: standard 4690 / featured 604 / basic 1259 / stub 116
  - 缺 dynasty: 4231（24.3%）
  - 正文平均: 898B

- **候选池（ic⊆bp）**: 217页待清除重复PN，513条可移除
  - ic=4: 3页 / ic=3: 58页 / ic=2: 81页 / ic=1: 56页
  - ic=1/2 合计137页（63%池），低水位主导
  - ic⊈bp（blockquote未覆盖inline PN）: 693条，需另案处理

- **暂存状态**: 261文件，748新增，559删减（自R9603发布后积压）
- **最近发布**: R9603（91ddd3b69），距下次R9700（%97=0）尚有43轮

- **观察A**: 池从R9628的287页降至217页（-24%），ic=4仅剩3页
- **观察B**: ic=1/2占63%，可扩大批量至8-10页/轮加速消耗
- **观察C**: 其他实例 add-blockquote 仍在补充池，但补给速度低于消耗
- **观察D**: H20/H21/H22 全部收敛，周期任务正常
- **观察E**: 缺dynasty 4231页为最大元数据缺口

- **建议**:
  1. 扩大 ic⊆bp 批量至8-10页/轮，加速低水位消耗
  2. R9700 执行 /wiki publish 清积压
  3. dynasty 批量填充仍待规划

### W5 反思 (R9628, %29=0)
> 全库状态扫描与模式分析（上次W5: R9593，间隔35轮）

- **轮次**: R9593—R9627, 34轮, accept 34/34 (100%)
  - ic⊈bp partial cleanup（主导，稳定 5页/轮 清除重复 inline PN）
  - H22 fix-emdash×2（R9608 2/50, R9622 0/50 → 完全收敛）
  - 周期任务全部正常执行

- **库规模**: 17422页（较R9593的17393页+29）
  - quality: standard 4690 / featured 604 / basic 1259 / stub 116
  - 非章节页: 17128
  - 缺 dynasty: 4231（24.3%）
  - 正文平均: 898B

- **候选池（ic⊆bp）**: 287页待清除重复PN，775条可移除
  - ic=4: 55页 / ic=3: 76页 / ic=2: 81页 / ic=1: 56页
  - ic=1/2 合计137页（48%池），过半存量已进入低水位
  - ic⊈bp（blockquote未覆盖inline PN）: 874条，需另案处理

- **暂存状态**: 88文件，391新增，253删减（自R9603发布后积压）
- **最近发布**: R9603（91ddd3b69），距下次R9700（%97=0）尚有72轮

- **观察A**: ic⊆bp 池从R9593的397页降至287页（-28%），持续消耗中
- **观察B**: ic=1/2低水位页已占48%，后续可加速消耗（每轮多页批量处理）
- **观察C**: 其他实例的 add-blockquote 操作仍在补充池
- **观察D**: H22完全收敛（0修改），周期任务全部正常
- **观察E**: 缺dynasty 4231页（24.3%）为最大元数据缺口

- **建议**:
  1. 继续 ic⊈bp partial cleanup，优先消耗 ic=4/3 页
  2. ic=1/2 页可逐步扩大到 8-10页/轮 批量清理
  3. R9700 执行 /wiki publish 清积压
  4. dynasty 批量填充仍待规划
...
### W5 反思 (R9593, >50轮强制)
> 全库状态扫描与模式分析（上次W5: R9541，间隔52轮）

- **轮次**: R9541—R9593, 52轮, accept 52/52 (100%)
  - enrich-quality×48（ic⊈bp partial cleanup 为主导，~30条/轮清除重复inline PN）
  - H21 person-table-scan×2（零错误）
  - H22 fix-emdash×1（0修改，已收敛）
  - 其他: H20/H23/H24 未触发

- **库规模**: 17393页（较R9312的17261页+132）
  - quality: standard 4690 / featured 604 / basic 1259 / none 10840
  - 新增页面为 create-page 轮次产出（同三品/参知政事/枢密院/尚书台等官职页）

- **候选池（ic⊆bp）**: ~397页待清除重复PN，~1268条可移除
  - ic⊈bp（blockquote未覆盖inline PN）: ~3423条，需另案处理
  - 池中大部分页面已降至 +5～+6 条（低水位），接近消耗完毕

- **暂存状态**: 234文件, 1687新增, 717删减（跨多轮未发布积压）
- **最近发布**: R9509（e21b4f885），距下次 R9603（%97=0）尚有~10轮

- **观察A**: ic⊈bp partial cleanup 稳定消耗中，池从 529→397 页（-25%）
- **观察B**: 但其他 刘攽 add-blockquote 操作持续补充池（备御+8等反复出现）
- **观察C**: create-page 任务恢复（R9588 同三品/参知政事, R9591 枢密院/尚书台）
- **观察D**: H20/H21/H22 均已达收敛，H23/H24 未激活（housekeeping_queue H-P2 空）
- **观察E**: 距上次发布~87轮，积压234文件待清

- **建议**:
  1. 继续 ic⊈bp 消耗至池 < 300 页后转向 ic⊈bp 覆盖策略
  2. R9603（%97=0）需执行 /wiki publish 清积压
  3. 可考虑 H23/H24 merge 任务激活（housekeeping_queue 写入任务项）
...
### W5 反思 (R10025→R10077, +52轮强制)
> 库状态扫描与模式分析（距上次W5已52轮，触发>50轮强制W5）

- **轮次**: R10025→R10077, 52轮, accept 60/60 (100%)
  - 刘攽(W10e enrich): 32轮 (53.3%), ~480页 enriched (+3-5条blockquote/页)
  - 范祖禹(0bq→bq enrich): 28轮 (46.7%), ~140页
  - H20/H22周期任务全部收敛（0修改）

- **库规模**: 17422页（冻结）
  - quality: standard 4690 / featured 604 / basic 1339 / stub 86 / none 10703
  - **quality分布完全未变** — W10e enrich不修改quality字段
  - 正文平均: 965B/页（较R10025的~774B显著增长，因blockquote内容增加）
  - 有blockquote的页面: 4509（较之前持续增长）
  - 有PN无blockquote: 仅1页（几近全cover）

- **模式判定**:
  - **模式B（质量停滞）**: quality层级冻结，enrich不提升quality标签
  - **模式C（深度不足）**: 无新featured页，W10e重在广度覆盖
  - **模式F（稳定运行）**: 100% accept率，两实例和谐共存

- **建议**: W10e广度覆盖仍有空间（命中率16-31%稳定），继续当前策略；待R10097发布轮commit + push

### W5 反思 (R10093→R10121, +28轮，%29=0强制)
> 广度持续扩张，quality指标固化
- **轮次**: R10093→R10121, 28轮, accept 28/28 (100%)
  - 刘攽(W10e enrich): 16轮, ~160页 enriched with PN blockquotes
  - 范祖禹(0bq→bq enrich): 12轮, ~60页 person pages enriched
- **质量分布**（与R10092完全一致，未变化）:
  - premium: 0 / featured: 604 / standard: 4690 / basic: 1339 / stub: 86 / none: 10409
- **周期任务**: H20/H21/H22全部收敛（0修改），H17/H18无脚本跳过
- **命中率趋势**: W10e候选池充足，每80页扫描命中12-18页（15%-23%），稳定

**模式判定**: 模式B（质量停滞）持续 — quality字段冻结，广度增益不计入QA指标
**建议**: 保持当前W10e策略，候选池深度足够（每轮12-18/80），继续按batch=10推进
